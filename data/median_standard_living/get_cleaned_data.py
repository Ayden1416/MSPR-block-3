# -*- coding: utf-8 -*-
import os
import sys
import pandas as pd

#===================================================#
#               PATH CONFIGURATION                  #
#===================================================#
BASE_PATH_SCRIPT = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.abspath(os.path.join(BASE_PATH_SCRIPT, "..", ".."))

if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

from const import (
    MEDIAN_INCOME_2017_DATASET,
    MEDIAN_INCOME_2022_DATASET
)

#===================================================#
#              CONSTANTS & MAPPINGS                  #
#===================================================#

# Map raw department names to their INSEE codes
DEPARTMENT_CODE_MAP = {
    'Ain': '01', 'Aisne': '02', 'Allier': '03', 'Alpes-de-Haute-Provence': '04', 'Hautes-Alpes': '05',
    'Alpes-Maritimes': '06', 'Ardèche': '07', 'Ardennes': '08', 'Ariège': '09', 'Aube': '10',
    'Aude': '11', 'Aveyron': '12', 'Bouches-du-Rhône': '13', 'Calvados': '14', 'Cantal': '15',
    'Charente': '16', 'Charente-Maritime': '17', 'Cher': '18', 'Corrèze': '19', 'Corse-du-Sud': '2A',
    'Haute-Corse': '2B', "Côte-d'Or": '21', "Côtes-d'Armor": '22', 'Creuse': '23', 'Dordogne': '24',
    'Doubs': '25', 'Drôme': '26', 'Eure': '27', 'Eure-et-Loir': '28', 'Finistère': '29',
    'Gard': '30', 'Haute-Garonne': '31', 'Gers': '32', 'Gironde': '33', 'Hérault': '34',
    'Ille-et-Vilaine': '35', 'Indre': '36', 'Indre-et-Loire': '37', 'Isère': '38', 'Jura': '39',
    'Landes': '40', 'Loir-et-Cher': '41', 'Loire': '42', 'Haute-Loire': '43', 'Loire-Atlantique': '44',
    'Loiret': '45', 'Lot': '46', 'Lot-et-Garonne': '47', 'Lozère': '48', 'Maine-et-Loire': '49',
    'Manche': '50', 'Marne': '51', 'Haute-Marne': '52', 'Mayenne': '53', 'Meurthe-et-Moselle': '54',
    'Meuse': '55', 'Morbihan': '56', 'Moselle': '57', 'Nièvre': '58', 'Nord': '59',
    'Oise': '60', 'Orne': '61', 'Pas-de-Calais': '62', 'Puy-de-Dôme': '63', 'Pyrénées-Atlantiques': '64',
    'Hautes-Pyrénées': '65', 'Pyrénées-Orientales': '66', 'Bas-Rhin': '67', 'Haut-Rhin': '68', 'Rhône': '69',
    'Haute-Saône': '70', 'Saône-et-Loire': '71', 'Sarthe': '72', 'Savoie': '73', 'Haute-Savoie': '74',
    'Paris': '75', 'Seine-Maritime': '76', 'Seine-et-Marne': '77', 'Yvelines': '78', 'Deux-Sèvres': '79',
    'Somme': '80', 'Tarn': '81', 'Tarn-et-Garonne': '82', 'Var': '83', 'Vaucluse': '84',
    'Vendée': '85', 'Vienne': '86', 'Haute-Vienne': '87', 'Vosges': '88', 'Yonne': '89',
    'Territoire de Belfort': '90', 'Essonne': '91', 'Hauts-de-Seine': '92', 'Seine-Saint-Denis': '93',
    'Val-de-Marne': '94', "Val-d'Oise": '95', 'Guadeloupe': '971', 'Martinique': '972',
    'Guyane': '973', 'La Réunion': '974', 'Mayotte': '976'
}

#===================================================#
#              PROCESSING FUNCTIONS                 #
#===================================================#

def get_cleaned_data():
    """
    Loads and cleans median income data from Excel datasets for 2017 and 2022.
    
    Returns a dictionary structured as follows:
    {
        '2017': {departement_code: average_median_income_2017, ...},
        '2022': {departement_code: average_median_income_2022, ...}
    }
    """
    #----------------- DATA LOADING -----------------#
    df_2017 = pd.read_excel(MEDIAN_INCOME_2017_DATASET)
    df_2022 = pd.read_excel(MEDIAN_INCOME_2022_DATASET)
    
    #----------------- COLUMN STANDARDIZATION -----------------#
    rename_map = {
        'Libellé': 'departement',
        'Département': 'departement',
        'Code Département': 'departement',
        'Niveau de vie mediant': 'niveau_vie_median',
        'Niveau de vie annuel médian (euros)': 'niveau_vie_median',
        'Revenu mediant des habitants': 'niveau_vie_median'
    }
    df_2017 = df_2017.rename(columns=rename_map)
    df_2022 = df_2022.rename(columns=rename_map)

    # Ensure required columns exist
    for year, df in [('2017', df_2017), ('2022', df_2022)]:
        if 'departement' not in df.columns or 'niveau_vie_median' not in df.columns:
            raise ValueError(f"Missing required columns in {year} data: {df.columns.tolist()}")

    #----------------- DEPARTMENT CODE MAPPING -----------------#
    # Convert department names to codes; if already a code, leave unchanged
    def map_dept(value):
        return DEPARTMENT_CODE_MAP.get(value, value)

    df_2017['departement'] = df_2017['departement'].apply(map_dept)
    df_2022['departement'] = df_2022['departement'].apply(map_dept)

    # Drop rows with unmapped or missing values
    df_2017 = df_2017.dropna(subset=['departement', 'niveau_vie_median'])
    df_2022 = df_2022.dropna(subset=['departement', 'niveau_vie_median'])

    #----------------- AGGREGATION -----------------#
    result = {'2017': {}, '2022': {}}

    for year, df in [('2017', df_2017), ('2022', df_2022)]:
        means = (
            df
            .groupby('departement')['niveau_vie_median']
            .mean()
            .round(3)
            .astype(float)
        )
        result[year] = means.to_dict()

    return result

#===================================================#
#                 MAIN EXECUTION                    #
#===================================================#

if __name__ == "__main__":
    print(get_cleaned_data())